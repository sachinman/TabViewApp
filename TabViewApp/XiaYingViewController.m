...
@interface XiaYingViewController ()
{
...
@implementation XiaYingViewController
@synthesize shanglianField=_shanglianField;
...
- (void)viewDidLoad
{
    ...
    //[_shanglianField becomeFirstResponder];
    ...
    self.navigationItem.title=@"下联应对";
    _shanglianField.delegate=self;
...
-(void)shengchengDuilian
{
	...
	[NSString stringWithFormat:
	 ...
	 "</GetDownCouplet>"
	 "</soap:Body>"
	 "</soap:Envelope>",  _shanglianField.text
	 ];
    ...
    if ([elementName isEqualToString:@"GetDownCoupletResult"]) {
        ...
        if ([fenlian count]>1) {
            ...
            if (_childController==nil) {
                _childController=[[ResultViewController alloc]initWithNibName:@"ResultViewController"bundle:nil];
            }
            _childController.title=@"生成结果";
            NSString *duilianResult=[[NSString alloc]initWithFormat:@"%@|%@",_shanglianField.text,soapResults];
            
            _childController.message=duilianResult;
            
            [self openDB];
            [self insertRecordIntoTableNamed:@"Couplets" withField1:@"class" field1Value:@"下联应对" andField2:@"input" field2Value:_shanglianField.text andField3:@"output" field3Value:duilianResult];
            sqlite3_close(db);
...
- (void)viewDidUnload {
    [self setShanglianField:nil];
    [super viewDidUnload];
}

-(IBAction)backgroundTap:(id)sender
{
    [_shanglianField resignFirstResponder];
}
